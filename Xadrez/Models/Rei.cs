namespace Xadrez.Models; // Define o namespace onde a classe Rei está localizada

using Xadrez.Models.Pecas; // Importa o namespace necessário para acessar outras peças, como Torre
...
{
    /// <summary>
    /// Propriedade que indica se o Rei está em cheque.
    /// </summary>
    public bool EmCheque { get; set; }

    /// <summary>
    /// Propriedade que indica se o Rei já se moveu durante a partida.  
    /// </summary>
    public bool JaMoveu { get; set; } = false;

    /// <summary>
    /// Construtor da classe Rei, define a cor da peça.
    /// </summary>
    /// <param name="eBranca"> Indica se a peça é branca.</param>
    public Rei(bool eBranca)
    {
        EBranca = eBranca; // Inicializa a propriedade EBranca
    }

    /// <summary>
    /// Propriedade que indica se a peça é branca. Sobrescreve a propriedade da classe base.
    /// </summary>
    public override bool EBranca { get; }
    ...
    public bool VerificaRoque(Tabuleiro tabuleiro, bool roquePequeno = false)
    {
        ...
        if (casaRei?.Peca is not Rei || casaTorre?.Peca is not Torre torre || torre.EBranca != EBranca || torre.JaMoveu)
            return false;
    ...
    public Movimento ExecutaRoque(Tabuleiro tabuleiro, bool roquePequeno = false)
    {
        ...
        casaRei.Peca = null;
        JaMoveu = true; // Marca que o rei já moveu

        // Move a torre para a nova casa
        novaCasaTorre.Peca = torre;
        casaTorre.Peca = null;
        torre!.JaMoveu = true; // Marca que a torre já moveu
